...
const AppStatusTable = ({ tableData }) => {
        ...
        <tbody>
          {currentData.map((item) => (
            <tr key={item.id}>
              ...
              <td>{item.status}</td>
              <td>{item.priority}</td>
              <td onClick={() => navigate(`/status/update/${item.id}`, { state: { id: item.id, status: item.status, priority: item.priority, } })}>
                <Link className="icon-font"

                // to={`/status/update/${item.id}`}
