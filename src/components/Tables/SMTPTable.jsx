...
const SMTPTable = ({ tableData }) => {
  ...
  const handleSort = (column) => {
    ...
    const sorted = [...sortedData].sort((a, b) => {
      if (column === "app") {
        return (
          a.apps.localeCompare(b.apps) * (sortDirection === "asc" ? 1 : -1)
        );
      }
      return 0;
    });
  ...
  return (
    <>
      ...
      >
        <thead>
          <tr>
            <th style={{ cursor: "pointer" }} onClick={() => handleSort("app")}>
              App
              {sortColumn === "app" && (
