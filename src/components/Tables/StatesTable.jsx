...
const StatesTable = ({ tableData }) => {
  ...
  const filteredData = sortedData.filter((item) =>
    item.states.toLowerCase().includes(filterText.toLowerCase())
  );
  ...
  const handleSort = (column) => {
    ...
    const sorted = [...sortedData].sort((a, b) => {
      if (column === "states") {
        return (
          a.apps.localeCompare(b.states) * (sortDirection === "asc" ? 1 : -1)
        );
      }
      return 0;
    });
  ...
  const deleteHandler = async (id) => {
    try {
      let res = await StateServices.deleteState(id)
      if (res.data.status) {
        ...
        <tbody>
          {currentData.map((item, index) => (
            <tr key={item.id}>
              ...
              <td>{item.states}</td>
              <td  >
                <div className="d-inline-block me-2" onClick={() => navigate(`/states/update/${item.id}`, { state: { id: item.id, state: item.states } })}>

                  <Link className="icon-font"

                  // to={`/states/update/${item.id}`}
