...
const TaxTable = ({ tableData }) => {
  ...
  const filteredData = sortedData.filter((item) =>
    item.value.includes(filterText.toLowerCase())
  );
  ...
  const handleSort = (column) => {
    ...
    const sorted = [...sortedData].sort((a, b) => {
      if (column === "value") {
        return (
          a.value.localeCompare(b.states) * (sortDirection === "asc" ? 1 : -1)
        );
      }
      return 0;
    });
  ...
  const deleteHandler = async (id) => {
    try {
      let res = await TaxServices.deleteTax(id);
      if (res.data.status) {
            ...
            <tr key={item.id}>
              ...
              <td>
                <div
                  ...
                  onClick={() =>
                    navigate(`/tax-management/update/${item.id}`, {
                      state: {
                        id: item.id,
                        desc: item.desc,
                        value: item.value,
